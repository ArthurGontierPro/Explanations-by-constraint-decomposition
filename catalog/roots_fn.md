...
> explain. It is not a list of all valid explanations of `roots_fn`,
> and no claim of that kind is made anywhere in this catalog.** See
> `catalog/README.md`, "What the catalog claims".

**This entry is a pointer.** `roots_fn` is the **functional form** of
[`roots`](roots.md) and is not a separate constraint. Everything this
method has to say about it is in [`catalog/roots.md`](roots.md); nothing is duplicated here,
...
| | |
|---|---|
| **Tier** | **unclassified** (`- unclassified`) — `python3 tools/mzn_coverage.py --rank`, 2026-09-21. **This is an artifact of a blank cell, not a property of the constraint** — see below. The base `roots` ranks **out of scope** (`- out of scope`) |
| **Status** | **resolves to [`roots.md`](roots.md)**, whose Status row reads `nothing generated — blocked on G8` (read off that file, 2026-09-21). **No status-legend value is asserted for `roots_fn` itself**, and this entry generates nothing of its own |
| **Generated** | **0** — there is no `cata/roots_fn.tex`, and there should not be one. The rules for this constraint are the rules in [`roots.md`](roots.md) |
...
| **Calibration** | **resolves to [`roots.md`](roots.md)**. `CHRISTMAS_LIST.md:217` records the literature column for this row as `—`; that is a statement about the *row*, not a finding that no paper explains roots, so `no published rule exists` is **not** claimed here |
| **Last measured** | 2026-09-21, `python3 tools/mzn_coverage.py --rank --json`, `make validate`, `ls cata/`, and reads of `CHRISTMAS_LIST.md:217`, `CHRISTMAS_LIST.md:206` and `catalog/roots.md` |

## Constraint

`roots_fn(...)` — **not vendored in this repo.** The MiniZinc library is not checked in:
`tools/data/minizinc-2.10.1-globals.txt:109` carries the *name* `roots_fn` and nothing else,
...
Read with the `roots` row at `CHRISTMAS_LIST.md:206`, that says `roots_fn` introduces a
result — one `var set of int`: the preimage under `x` of the value set `t` — and posts `roots` over it.

**Provenance of the name:** `tools/data/minizinc-2.10.1-globals.txt:109`.
...
2. **A pointer is a finding, and an absence is not.** Saying "`roots_fn` is `roots` under
   another syntax, so its explanation is `roots`'s" is a checkable claim. Leaving eleven files
...
3. `find`'s AND/OR traversal, `an`'s DNF flattening and the LaTeX printer are functions of that
   ...
   `cata/roots_fn.tex` to contain that `cata/`'s `roots` artifact does not already contain.**
...
result were not already determined by the other arguments — but for `roots_fn` it is
(the preimage set is a function of `x` and `t`), so the functional form adds no literal and no explanation. *This is reasoning
...
it. This entry supplies the missing route and its reason: **the route of `roots_fn` is the route
of `roots`**, because it is the same decomposition. `CHRISTMAS_LIST.md:206`'s route cell
for `roots` reads, verbatim:

> **E5**. Note: MiniZinc's `range`/`roots` take `var set of int` and have **no library decomposition** (declared solver-native).
...
their bases are spread across **A**, **B**, **C** and **out of scope**. `roots` is
...
no paper explains this constraint. The citation that applies is `roots`'s, at
`CHRISTMAS_LIST.md:206`, and it is quoted in [`roots.md`](roots.md).

**Rule shape:** see [`roots.md`](roots.md). Nothing about any paper's content is stated
...
**Source: the `roots` row, `CHRISTMAS_LIST.md:206`** — not the `*_fn` row, whose solver
...
world: a solver never sees `roots_fn`, it sees the `roots` the function posts, so the
...
`roots_fn` and should not: the decomposition to encode is `roots`'s.
**Emitted by:** nothing — `ls cata/` (2026-09-21) lists 16 `.tex` files and none is
`roots_fn.tex`.
**Spec:** none — there is no `decomps/roots_fn.md`, and the 43 specs under `decomps/` are
named for predicate forms. [`roots.md`](roots.md) records whether `roots` has one.
...
invoked for `roots_fn`, so there is no `%% generator diagnostics (W1-T3)` footer to read.
...
`roots`, and [`roots.md`](roots.md)'s **Scope of this entry** section states it, including
the events that came back empty. **This entry adds no scope of its own and claims none.**

## Generated rules

**None under this name**, by design rather than by failure: `grep -o '\\frac' cata/roots_fn.tex`
has no file to read. The generated rules for this constraint are rendered in
[`roots.md`](roots.md).

## Status

**resolves to [`roots.md`](roots.md)** — that entry's Status row reads `nothing generated — blocked on G8`.
...
name. What **is** established, and is this entry's content, is the resolution itself: `roots_fn`
is `roots` in functional syntax, the decomposition and therefore the explanation are the same
object, and the argument for that is above with its two qualifications attached.

## Calibration (W3-T5, D-0013)

**Verdict: resolves to [`roots.md`](roots.md).**

No comparison is stated here and none should be: with no rule generated under this name there is
no premise to place in an implication order, and the published side is `roots`'s. Note in
...
Whatever verdict [`roots.md`](roots.md) reaches is `roots_fn`'s verdict, for the same reason
...
| (base's) | whatever [`roots.md`](roots.md)'s Gaps table lists for `roots` applies here unchanged |
...
E-code from it. R2's reading, above: the applicable route is `roots`'s, quoted verbatim in
"The `*_fn` decision" from `CHRISTMAS_LIST.md:206`.
Source: `docs/DECOMP_FORMAT_NOTES.md` (consolidated wave-two numbering) for what the numbered
gaps mean; none is attributed to `roots_fn` itself.

## How this entry was produced

- `python3 tools/mzn_coverage.py --rank --json` (2026-09-21) → `roots_fn` in `- unclassified`,
  no E-codes parsed, `CHRISTMAS_LIST.md` line 217; and `roots` in `- out of scope`.
...
- `CHRISTMAS_LIST.md:206` read → the `roots` row: the solver cells in the table above and
  the route cell quoted verbatim in "The `*_fn` decision".
- `CHRISTMAS_LIST.md:106-109` read → the solver-column legend.
- `tools/data/minizinc-2.10.1-globals.txt:109` read → the name, and that the snapshot carries
  names only.
- `catalog/roots.md` read, not run → the base entry's Status row, quoted above as
  `nothing generated — blocked on G8`, and the fact that it carries the rules, scope and calibration this entry defers to.
- `ls cata/` → 16 `.tex` files, none named `roots_fn.tex`.
- `make validate` (run 2026-09-21, output redirected to a file then grepped, per `CLAUDE.md`
  ...
  that run mentions `roots_fn`, which is the measurement behind the **Validator** row.
