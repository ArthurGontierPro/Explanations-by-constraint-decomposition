...
> explain. It is not a list of all valid explanations of `nvalue_fn`,
> and no claim of that kind is made anywhere in this catalog.** See
> `catalog/README.md`, "What the catalog claims".

**This entry is a pointer.** `nvalue_fn` is the **functional form** of
[`nvalue`](nvalues.md) and is not a separate constraint. Everything this
method has to say about it is in [`catalog/nvalues.md`](nvalues.md); nothing is duplicated here,
...
| | |
|---|---|
| **Tier** | **unclassified** (`- unclassified`) — `python3 tools/mzn_coverage.py --rank`, 2026-09-21. **This is an artifact of a blank cell, not a property of the constraint** — see below. The base `nvalue` ranks **A** (`A no-literature + solver-decomposes`) |
| **Status** | **resolves to [`nvalues.md`](nvalues.md)**, whose Status row reads `flagged` (read off that file, 2026-09-21). **No status-legend value is asserted for `nvalue_fn` itself**, and this entry generates nothing of its own |
| **Generated** | **0** — there is no `cata/nvalue_fn.tex`, and there should not be one. The rules for this constraint are the rules in [`nvalues.md`](nvalues.md) |
...
| **Calibration** | **resolves to [`nvalues.md`](nvalues.md)**. `CHRISTMAS_LIST.md:217` records the literature column for this row as `—`; that is a statement about the *row*, not a finding that no paper explains nvalue, so `no published rule exists` is **not** claimed here |
| **Last measured** | 2026-09-21, `python3 tools/mzn_coverage.py --rank --json`, `make validate`, `ls cata/`, and reads of `CHRISTMAS_LIST.md:217`, `CHRISTMAS_LIST.md:130` and `catalog/nvalues.md` |

## Constraint

`nvalue_fn(...)` — **not vendored in this repo.** The MiniZinc library is not checked in:
`tools/data/minizinc-2.10.1-globals.txt:98` carries the *name* `nvalue_fn` and nothing else,
...
Read with the `nvalue` row at `CHRISTMAS_LIST.md:130`, that says `nvalue_fn` introduces a
result — one `var int`: how many distinct values the `x[i]` take — and posts `nvalue` over it.

**Provenance of the name:** `tools/data/minizinc-2.10.1-globals.txt:98`.
...
2. **A pointer is a finding, and an absence is not.** Saying "`nvalue_fn` is `nvalue` under
   another syntax, so its explanation is `nvalue`'s" is a checkable claim. Leaving eleven files
...
3. `find`'s AND/OR traversal, `an`'s DNF flattening and the LaTeX printer are functions of that
   ...
   `cata/nvalue_fn.tex` to contain that `cata/`'s `nvalue` artifact does not already contain.**
...
result were not already determined by the other arguments — but for `nvalue_fn` it is
(the number of distinct values is a function of `x`), so the functional form adds no literal and no explanation. *This is reasoning
...
it. This entry supplies the missing route and its reason: **the route of `nvalue_fn` is the route
of `nvalue`**, because it is the same decomposition. `CHRISTMAS_LIST.md:130`'s route cell
for `nvalue` reads, verbatim:

> **E0** — already in `cata/nvalues.tex`, but the generated rule repeats binders (`∀i` twice); fix is index hygiene, not an extension
...
their bases are spread across **A**, **B**, **C** and **out of scope**. `nvalue` is
...
no paper explains this constraint. The citation that applies is `nvalue`'s, at
`CHRISTMAS_LIST.md:130`, and it is quoted in [`nvalues.md`](nvalues.md).

**Rule shape:** see [`nvalues.md`](nvalues.md). Nothing about any paper's content is stated
...
**Source: the `nvalue` row, `CHRISTMAS_LIST.md:130`** — not the `*_fn` row, whose solver
...
world: a solver never sees `nvalue_fn`, it sees the `nvalue` the function posts, so the
...
`nvalue_fn` and should not: the decomposition to encode is `nvalue`'s.
**Emitted by:** nothing — `ls cata/` (2026-09-21) lists 16 `.tex` files and none is
`nvalue_fn.tex`.
**Spec:** none — there is no `decomps/nvalue_fn.md`, and the 43 specs under `decomps/` are
named for predicate forms. [`nvalues.md`](nvalues.md) records whether `nvalue` has one.
...
invoked for `nvalue_fn`, so there is no `%% generator diagnostics (W1-T3)` footer to read.
...
`nvalue`, and [`nvalues.md`](nvalues.md)'s **Scope of this entry** section states it, including
the events that came back empty. **This entry adds no scope of its own and claims none.**

## Generated rules

**None under this name**, by design rather than by failure: `grep -o '\\frac' cata/nvalue_fn.tex`
has no file to read. The generated rules for this constraint are rendered in
[`nvalues.md`](nvalues.md).

## Status

**resolves to [`nvalues.md`](nvalues.md)** — that entry's Status row reads `flagged`.
...
name. What **is** established, and is this entry's content, is the resolution itself: `nvalue_fn`
is `nvalue` in functional syntax, the decomposition and therefore the explanation are the same
object, and the argument for that is above with its two qualifications attached.

## Calibration (W3-T5, D-0013)

**Verdict: resolves to [`nvalues.md`](nvalues.md).**

No comparison is stated here and none should be: with no rule generated under this name there is
no premise to place in an implication order, and the published side is `nvalue`'s. Note in
...
Whatever verdict [`nvalues.md`](nvalues.md) reaches is `nvalue_fn`'s verdict, for the same reason
...
| (base's) | whatever [`nvalues.md`](nvalues.md)'s Gaps table lists for `nvalue` applies here unchanged |
...
E-code from it. R2's reading, above: the applicable route is `nvalue`'s, quoted verbatim in
"The `*_fn` decision" from `CHRISTMAS_LIST.md:130`.
Source: `docs/DECOMP_FORMAT_NOTES.md` (consolidated wave-two numbering) for what the numbered
gaps mean; none is attributed to `nvalue_fn` itself.

## How this entry was produced

- `python3 tools/mzn_coverage.py --rank --json` (2026-09-21) → `nvalue_fn` in `- unclassified`,
  no E-codes parsed, `CHRISTMAS_LIST.md` line 217; and `nvalue` in `A no-literature + solver-decomposes`.
...
- `CHRISTMAS_LIST.md:130` read → the `nvalue` row: the solver cells in the table above and
  the route cell quoted verbatim in "The `*_fn` decision".
- `CHRISTMAS_LIST.md:106-109` read → the solver-column legend.
- `tools/data/minizinc-2.10.1-globals.txt:98` read → the name, and that the snapshot carries
  names only.
- `catalog/nvalues.md` read, not run → the base entry's Status row, quoted above as
  `flagged`, and the fact that it carries the rules, scope and calibration this entry defers to.
- `ls cata/` → 16 `.tex` files, none named `nvalue_fn.tex`.
- `make validate` (run 2026-09-21, output redirected to a file then grepped, per `CLAUDE.md`
  ...
  that run mentions `nvalue_fn`, which is the measurement behind the **Validator** row.
