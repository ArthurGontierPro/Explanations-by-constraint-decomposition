...
> explain. It is not a list of all valid explanations of `bounded_path`, and no claim
...
| **Generated** | no generator entry — there is no `cata/bounded_path.tex` |
| **Validator** | out of scope — no `cata/bounded_path.tex` exists for `make validate` to check |
...
`bounded_path` — no MiniZinc signature is vendored in this repo.

**Provenance of the name:** `tools/data/minizinc-2.10.1-globals.txt` lists `bounded_path` as a
...
`bounded_path`, and none is written here (`catalog/TEMPLATE.md`: never state a rule shape from
memory).
...
**None.** There is no `decomps/bounded_path.md` and no `cata/bounded_path.tex`; no decomposition in
this project's format has been written for `bounded_path`.
...
**Is that classification right for `bounded_path`?** Yes. Every constraint in this group has a **native** Chuffed propagator built on a dedicated graph algorithm (see Solver support), and the published explanations that exist (Francis & Stuckey for `circuit`/`subcircuit`) are reachability arguments over the run-time graph, not over a finite unrolled Boolean formula. This is not a section-filter artefact: the route cell itself says `E6`, and E6 is one of the three categories `docs/ROADMAP.md:106` names explicitly.
...
None. No `cata/bounded_path.tex` exists, so there is nothing to render.

## Status

**`nothing generated — blocked on E6`**

No decomposition has been attempted, encoded or generated for `bounded_path`. This is a scope
...
- `python3 tools/mzn_coverage.py --rank` (run 2026-09-21) → `bounded_path` under tier `- out of scope`,
...
- `os.path.isfile("decomps/bounded_path.md")` → False. `os.path.isfile("cata/bounded_path.tex")` → False.
