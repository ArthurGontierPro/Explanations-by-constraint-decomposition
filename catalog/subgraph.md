...
> explain. It is not a list of all valid explanations of `subgraph`, and no claim
...
| **Generated** | no generator entry — there is no `cata/subgraph.tex` |
| **Validator** | out of scope — no `cata/subgraph.tex` exists for `make validate` to check |
...
`subgraph` — no MiniZinc signature is vendored in this repo.

**Provenance of the name:** `tools/data/minizinc-2.10.1-globals.txt` lists `subgraph` as a
...
`subgraph`, and none is written here (`catalog/TEMPLATE.md`: never state a rule shape from
memory).
...
**None.** There is no `decomps/subgraph.md` and no `cata/subgraph.tex`; no decomposition in
this project's format has been written for `subgraph`.
...
**Is that classification right for `subgraph`?** Yes. Every constraint in this group has a **native** Chuffed propagator built on a dedicated graph algorithm (see Solver support), and the published explanations that exist (Francis & Stuckey for `circuit`/`subcircuit`) are reachability arguments over the run-time graph, not over a finite unrolled Boolean formula. This is not a section-filter artefact: the route cell itself says `E6`, and E6 is one of the three categories `docs/ROADMAP.md:106` names explicitly.
...
None. No `cata/subgraph.tex` exists, so there is nothing to render.

## Status

**`nothing generated — blocked on E6`**

No decomposition has been attempted, encoded or generated for `subgraph`. This is a scope
...
- `python3 tools/mzn_coverage.py --rank` (run 2026-09-21) → `subgraph` under tier `- out of scope`,
...
- `os.path.isfile("decomps/subgraph.md")` → False. `os.path.isfile("cata/subgraph.tex")` → False.
