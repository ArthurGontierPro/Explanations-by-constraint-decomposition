...
> explain. It is not a list of all valid explanations of `among_fn`,
> and no claim of that kind is made anywhere in this catalog.** See
> `catalog/README.md`, "What the catalog claims".

**This entry is a pointer.** `among_fn` is the **functional form** of
[`among`](among.md) and is not a separate constraint. Everything this
method has to say about it is in [`catalog/among.md`](among.md); nothing is duplicated here,
...
`among_fn(...)` — **not vendored in this repo.** The MiniZinc library is not checked in:
`tools/data/minizinc-2.10.1-globals.txt:24` carries the *name* `among_fn` and nothing else,
...
Read with the `among` row at `CHRISTMAS_LIST.md:129`, that says `among_fn` introduces a
result — one `var int`: how many of the `x[i]` take a value in the set `v` — and posts `among` over it.

**Provenance of the name:** `tools/data/minizinc-2.10.1-globals.txt:24`.
...
2. **A pointer is a finding, and an absence is not.** Saying "`among_fn` is `among` under
   another syntax, so its explanation is `among`'s" is a checkable claim. Leaving eleven files
...
3. `find`'s AND/OR traversal, `an`'s DNF flattening and the LaTeX printer are functions of that
   ...
   `cata/among_fn.tex` to contain that `cata/`'s `among` artifact does not already contain.**
...
result were not already determined by the other arguments — but for `among_fn` it is
(the count is a function of `x` and `v`), so the functional form adds no literal and no explanation. *This is reasoning
...
it. This entry supplies the missing route and its reason: **the route of `among_fn` is the route
of `among`**, because it is the same decomposition. `CHRISTMAS_LIST.md:129`'s route cell
for `among` reads, verbatim:

> **E0** — already in `cata/among.tex`
...
their bases are spread across **A**, **B**, **C** and **out of scope**. `among` is
...
no paper explains this constraint. The citation that applies is `among`'s, at
`CHRISTMAS_LIST.md:129`, and it is quoted in [`among.md`](among.md).

**Rule shape:** see [`among.md`](among.md). Nothing about any paper's content is stated
...
**Source: the `among` row, `CHRISTMAS_LIST.md:129`** — not the `*_fn` row, whose solver
...
world: a solver never sees `among_fn`, it sees the `among` the function posts, so the
...
`among_fn` and should not: the decomposition to encode is `among`'s.
**Emitted by:** nothing — `ls cata/` (2026-09-21) lists 16 `.tex` files and none is
`among_fn.tex`.
**Spec:** none — there is no `decomps/among_fn.md`, and the 43 specs under `decomps/` are
named for predicate forms. [`among.md`](among.md) records whether `among` has one.
...
invoked for `among_fn`, so there is no `%% generator diagnostics (W1-T3)` footer to read.
...
`among`, and [`among.md`](among.md)'s **Scope of this entry** section states it, including
the events that came back empty. **This entry adds no scope of its own and claims none.**

## Generated rules

**None under this name**, by design rather than by failure: `grep -o '\\frac' cata/among_fn.tex`
...
name. What **is** established, and is this entry's content, is the resolution itself: `among_fn`
is `among` in functional syntax, the decomposition and therefore the explanation are the same
object, and the argument for that is above with its two qualifications attached.

## Calibration (W3-T5, D-0013)

**Verdict: resolves to [`among.md`](among.md).**

No comparison is stated here and none should be: with no rule generated under this name there is
no premise to place in an implication order, and the published side is `among`'s. Note in
...
Whatever verdict [`among.md`](among.md) reaches is `among_fn`'s verdict, for the same reason
...
| (base's) | whatever [`among.md`](among.md)'s Gaps table lists for `among` applies here unchanged |
...
E-code from it. R2's reading, above: the applicable route is `among`'s, quoted verbatim in
"The `*_fn` decision" from `CHRISTMAS_LIST.md:129`.
Source: `docs/DECOMP_FORMAT_NOTES.md` (consolidated wave-two numbering) for what the numbered
gaps mean; none is attributed to `among_fn` itself.

## How this entry was produced

- `python3 tools/mzn_coverage.py --rank --json` (2026-09-21) → `among_fn` in `- unclassified`,
  no E-codes parsed, `CHRISTMAS_LIST.md` line 217; and `among` in `A no-literature + solver-decomposes`.
...
- `CHRISTMAS_LIST.md:129` read → the `among` row: the solver cells in the table above and
...
- `make validate` (run 2026-09-21, output redirected to a file then grepped, per `CLAUDE.md`
  ...
  that run mentions `among_fn`, which is the measurement behind the **Validator** row.
