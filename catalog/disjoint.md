...
> explain. It is not a list of all valid explanations of `disjoint`, and no claim
...
| **Generated** | no generator entry — there is no `cata/disjoint.tex` |
| **Validator** | out of scope — no `cata/disjoint.tex` exists for `make validate` to check |
...
`disjoint` — no MiniZinc signature is vendored in this repo.

**Provenance of the name:** `tools/data/minizinc-2.10.1-globals.txt` lists `disjoint` as a
...
`disjoint`, and none is written here (`catalog/TEMPLATE.md`: never state a rule shape from
memory).
...
**None.** There is no `decomps/disjoint.md` and no `cata/disjoint.tex`; no decomposition in
this project's format has been written for `disjoint`.
...
**Is that classification right for `disjoint`?** Yes, straightforwardly — this is the set-disjointness constraint proper, filed in "Set constraints" with route `E5` and no ambiguity.

**What would bring it in scope.** E5, the membership matrix (`CHRISTMAS_LIST.md:99`).

## Generated rules

None. No `cata/disjoint.tex` exists, so there is nothing to render.

## Status

**`nothing generated — blocked on E5`**

No decomposition has been attempted, encoded or generated for `disjoint`. This is a scope
...
- `python3 tools/mzn_coverage.py --rank` (run 2026-09-21) → `disjoint` under tier `- out of scope`,
...
- `os.path.isfile("decomps/disjoint.md")` → False. `os.path.isfile("cata/disjoint.tex")` → False.
