...
> explain. It is not a list of all valid explanations of `bin_packing`, and no claim
...
| **Generated** | no generator entry — there is no `cata/bin_packing.tex` |
| **Validator** | out of scope — no `cata/bin_packing.tex` exists |
...
`bin_packing` — no MiniZinc signature is vendored in this repo.
`tools/data/minizinc-2.10.1-globals.txt` lists `bin_packing` as a global of MiniZinc 2.10.1
...
No paper is sourced in `catalog/_literature/` for `bin_packing`.
...
**None.** There is no `decomps/bin_packing.md` and no `cata/bin_packing.tex` — nobody has written this
constraint's decomposition in this project's format yet, regardless of the scope question
below.

## Scope of this entry

**Why it is currently filed out of scope.** `tools/mzn_coverage.py --rank` puts `bin_packing`
...
None. No `cata/bin_packing.tex` exists.
...
- `python3 tools/mzn_coverage.py --rank` (run 2026-09-21) → `bin_packing` under tier `- out of scope`,
...
- `CHRISTMAS_LIST.md:227-244` read → the "What to actually ask for" priority list, which is
  where the misclassification argument above comes from (comparing `bin_packing`'s named extension
...
- `os.path.isfile("decomps/bin_packing.md")` → False. `os.path.isfile("cata/bin_packing.tex")` → False.
