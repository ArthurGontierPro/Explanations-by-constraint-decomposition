...
| **Validator** | not run — no `cata/bin_packing.tex` exists yet |
...
**None yet.** There is no `decomps/bin_packing.md` and no `cata/bin_packing.tex`. What it would take is
below.
...
`bin_packing`'s mechanism — weighted Boolean sums — is **the identical gap `knapsack` carries**
...
with `knapsack` and `cumulative`. Then someone writes `decomps/bin_packing.md` encoding bin
...
misclassification — `bin_packing`'s own route cell says `E8`, the same gap as tier-A `knapsack`,
not E5/E6/E7. The orchestrator checked that finding, recorded **D-0014**, and fixed the tool
(tier A count 36→43, out-of-scope 35→28). `bin_packing` is filed here under its corrected tier as
...
|---|---|
| `G11` | no weighted Boolean sum — `rule5/6/7` count occurrences, with no coefficients; `bin_packing`'s bin-capacity constraint needs per-item weights (`docs/DECOMP_FORMAT_NOTES.md:79`) |
...
- `python3 tools/mzn_coverage.py --rank` (re-run 2026-09-21, post-fix) → `bin_packing` under tier
...
- Nothing was compiled, run or validated. No rules exist yet for `bin_packing`; this entry states
