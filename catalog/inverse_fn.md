...
> explain. It is not a list of all valid explanations of `inverse_fn`,
> and no claim of that kind is made anywhere in this catalog.** See
> `catalog/README.md`, "What the catalog claims".

**This entry is a pointer.** `inverse_fn` is the **functional form** of
[`inverse`](inverse.md) and is not a separate constraint. Everything this
method has to say about it is in [`catalog/inverse.md`](inverse.md); nothing is duplicated here,
...
| **Last measured** | 2026-09-21, `python3 tools/mzn_coverage.py --rank --json`, `make validate`, `ls cata/`, and reads of `CHRISTMAS_LIST.md:217`, `CHRISTMAS_LIST.md:199` and `catalog/inverse.md` |

## Constraint

`inverse_fn(...)` — **not vendored in this repo.** The MiniZinc library is not checked in:
`tools/data/minizinc-2.10.1-globals.txt:73` carries the *name* `inverse_fn` and nothing else,
...
Read with the `inverse` row at `CHRISTMAS_LIST.md:199`, that says `inverse_fn` introduces a
result — an `array of var int`: the inverse permutation of `x` — and posts `inverse` over it.

**Provenance of the name:** `tools/data/minizinc-2.10.1-globals.txt:73`.
...
2. **A pointer is a finding, and an absence is not.** Saying "`inverse_fn` is `inverse` under
   another syntax, so its explanation is `inverse`'s" is a checkable claim. Leaving eleven files
...
3. `find`'s AND/OR traversal, `an`'s DNF flattening and the LaTeX printer are functions of that
   ...
   `cata/inverse_fn.tex` to contain that `cata/`'s `inverse` artifact does not already contain.**
...
result were not already determined by the other arguments — but for `inverse_fn` it is
(the inverse array is a function of `x`), so the functional form adds no literal and no explanation. *This is reasoning
...
it. This entry supplies the missing route and its reason: **the route of `inverse_fn` is the route
of `inverse`**, because it is the same decomposition. `CHRISTMAS_LIST.md:199`'s route cell
for `inverse` reads, verbatim:

> **E2** — channelling `x[i]=j <-> y[j]=i` is two reified families, close to **E0**
...
their bases are spread across **A**, **B**, **C** and **out of scope**. `inverse` is
**B**. The eleven are not unclassifiable; they are unclassified because a row that
...
no paper explains this constraint. The citation that applies is `inverse`'s, at
`CHRISTMAS_LIST.md:199`, and it is quoted in [`inverse.md`](inverse.md).

**Rule shape:** see [`inverse.md`](inverse.md). Nothing about any paper's content is stated
...
| | |
|---|---|
| Chuffed | native (`inverse` per Chuffed docs) |
| Geas | `[G]` present |
| Choco LCG | `[C]` present |

**Source: the `inverse` row, `CHRISTMAS_LIST.md:199`** — not the `*_fn` row, whose solver
...
world: a solver never sees `inverse_fn`, it sees the `inverse` the function posts, so the
...
`inverse_fn` and should not: the decomposition to encode is `inverse`'s.
**Emitted by:** nothing — `ls cata/` (2026-09-21) lists 16 `.tex` files and none is
`inverse_fn.tex`.
**Spec:** none — there is no `decomps/inverse_fn.md`, and the 43 specs under `decomps/` are
named for predicate forms. [`inverse.md`](inverse.md) records whether `inverse` has one.
...
invoked for `inverse_fn`, so there is no `%% generator diagnostics (W1-T3)` footer to read.
...
`inverse`, and [`inverse.md`](inverse.md)'s **Scope of this entry** section states it, including
the events that came back empty. **This entry adds no scope of its own and claims none.**

## Generated rules

**None under this name**, by design rather than by failure: `grep -o '\\frac' cata/inverse_fn.tex`
...
name. What **is** established, and is this entry's content, is the resolution itself: `inverse_fn`
is `inverse` in functional syntax, the decomposition and therefore the explanation are the same
object, and the argument for that is above with its two qualifications attached.

## Calibration (W3-T5, D-0013)

**Verdict: resolves to [`inverse.md`](inverse.md).**

No comparison is stated here and none should be: with no rule generated under this name there is
no premise to place in an implication order, and the published side is `inverse`'s. Note in
...
Whatever verdict [`inverse.md`](inverse.md) reaches is `inverse_fn`'s verdict, for the same reason
...
| (base's) | whatever [`inverse.md`](inverse.md)'s Gaps table lists for `inverse` applies here unchanged |
...
E-code from it. R2's reading, above: the applicable route is `inverse`'s, quoted verbatim in
"The `*_fn` decision" from `CHRISTMAS_LIST.md:199`.
Source: `docs/DECOMP_FORMAT_NOTES.md` (consolidated wave-two numbering) for what the numbered
gaps mean; none is attributed to `inverse_fn` itself.

## How this entry was produced

- `python3 tools/mzn_coverage.py --rank --json` (2026-09-21) → `inverse_fn` in `- unclassified`,
  no E-codes parsed, `CHRISTMAS_LIST.md` line 217; and `inverse` in `B no-literature + solver-native`.
...
- `CHRISTMAS_LIST.md:199` read → the `inverse` row: the solver cells in the table above and
...
- `make validate` (run 2026-09-21, output redirected to a file then grepped, per `CLAUDE.md`
  ...
  that run mentions `inverse_fn`, which is the measurement behind the **Validator** row.
