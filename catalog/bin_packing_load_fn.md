...
> explain. It is not a list of all valid explanations of `bin_packing_load_fn`,
> and no claim of that kind is made anywhere in this catalog.** See
> `catalog/README.md`, "What the catalog claims".

**This entry is a pointer.** `bin_packing_load_fn` is the **functional form** of
[`bin_packing_load`](bin_packing_load.md) and is not a separate constraint. Everything this
method has to say about it is in [`catalog/bin_packing_load.md`](bin_packing_load.md); nothing is duplicated here,
...
| **Last measured** | 2026-09-21, `python3 tools/mzn_coverage.py --rank --json`, `make validate`, `ls cata/`, and reads of `CHRISTMAS_LIST.md:217`, `CHRISTMAS_LIST.md:176` and `catalog/bin_packing_load.md` |

## Constraint

`bin_packing_load_fn(...)` — **not vendored in this repo.** The MiniZinc library is not checked in:
`tools/data/minizinc-2.10.1-globals.txt:35` carries the *name* `bin_packing_load_fn` and nothing else,
...
Read with the `bin_packing_load` row at `CHRISTMAS_LIST.md:176`, that says `bin_packing_load_fn` introduces a
result — an `array of var int`: the load of each bin — and posts `bin_packing_load` over it.

**Provenance of the name:** `tools/data/minizinc-2.10.1-globals.txt:35`.
...
2. **A pointer is a finding, and an absence is not.** Saying "`bin_packing_load_fn` is `bin_packing_load` under
   another syntax, so its explanation is `bin_packing_load`'s" is a checkable claim. Leaving eleven files
...
3. `find`'s AND/OR traversal, `an`'s DNF flattening and the LaTeX printer are functions of that
   ...
   `cata/bin_packing_load_fn.tex` to contain that `cata/`'s `bin_packing_load` artifact does not already contain.**
...
result were not already determined by the other arguments — but for `bin_packing_load_fn` it is
(each load is a function of the assignment `bin` and the weights `w`), so the functional form adds no literal and no explanation. *This is reasoning
...
it. This entry supplies the missing route and its reason: **the route of `bin_packing_load_fn` is the route
of `bin_packing_load`**, because it is the same decomposition. `CHRISTMAS_LIST.md:176`'s route cell
for `bin_packing_load` reads, verbatim:

> **E8** (was E3; weighted sums — D-0011); `_load` is the most tractable
...
no paper explains this constraint. The citation that applies is `bin_packing_load`'s, at
`CHRISTMAS_LIST.md:176`, and it is quoted in [`bin_packing_load.md`](bin_packing_load.md).

**Rule shape:** see [`bin_packing_load.md`](bin_packing_load.md). Nothing about any paper's content is stated
...
**Source: the `bin_packing_load` row, `CHRISTMAS_LIST.md:176`** — not the `*_fn` row, whose solver
...
world: a solver never sees `bin_packing_load_fn`, it sees the `bin_packing_load` the function posts, so the
...
`bin_packing_load_fn` and should not: the decomposition to encode is `bin_packing_load`'s.
**Emitted by:** nothing — `ls cata/` (2026-09-21) lists 16 `.tex` files and none is
`bin_packing_load_fn.tex`.
**Spec:** none — there is no `decomps/bin_packing_load_fn.md`, and the 43 specs under `decomps/` are
named for predicate forms. [`bin_packing_load.md`](bin_packing_load.md) records whether `bin_packing_load` has one.
...
invoked for `bin_packing_load_fn`, so there is no `%% generator diagnostics (W1-T3)` footer to read.
...
`bin_packing_load`, and [`bin_packing_load.md`](bin_packing_load.md)'s **Scope of this entry** section states it, including
the events that came back empty. **This entry adds no scope of its own and claims none.**

## Generated rules

**None under this name**, by design rather than by failure: `grep -o '\\frac' cata/bin_packing_load_fn.tex`
has no file to read. The generated rules for this constraint are rendered in
[`bin_packing_load.md`](bin_packing_load.md).
...
name. What **is** established, and is this entry's content, is the resolution itself: `bin_packing_load_fn`
is `bin_packing_load` in functional syntax, the decomposition and therefore the explanation are the same
object, and the argument for that is above with its two qualifications attached.

## Calibration (W3-T5, D-0013)

**Verdict: resolves to [`bin_packing_load.md`](bin_packing_load.md).**

No comparison is stated here and none should be: with no rule generated under this name there is
no premise to place in an implication order, and the published side is `bin_packing_load`'s. Note in
...
Whatever verdict [`bin_packing_load.md`](bin_packing_load.md) reaches is `bin_packing_load_fn`'s verdict, for the same reason
...
| (base's) | whatever [`bin_packing_load.md`](bin_packing_load.md)'s Gaps table lists for `bin_packing_load` applies here unchanged |
...
E-code from it. R2's reading, above: the applicable route is `bin_packing_load`'s, quoted verbatim in
"The `*_fn` decision" from `CHRISTMAS_LIST.md:176`.
Source: `docs/DECOMP_FORMAT_NOTES.md` (consolidated wave-two numbering) for what the numbered
gaps mean; none is attributed to `bin_packing_load_fn` itself.
...
- `CHRISTMAS_LIST.md:176` read → the `bin_packing_load` row: the solver cells in the table above and
...
- `make validate` (run 2026-09-21, output redirected to a file then grepped, per `CLAUDE.md`
  ...
  that run mentions `bin_packing_load_fn`, which is the measurement behind the **Validator** row.
