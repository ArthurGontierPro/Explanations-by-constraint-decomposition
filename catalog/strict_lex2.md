...
> explain. It is not a list of all valid explanations of `strict_lex2`, and no claim of that kind
> is made anywhere in this catalog.** See `catalog/README.md`, "What the catalog claims".

| | |
|---|---|
| **Tier** | **D** — `D literature + solver-native`, ecode `E0` (shared row with `lex_less`, `lex_lesseq`, `lex2`, `lex2_strict`) |
| **Status** | `nothing generated — blocked on G3` |
| **Generated** | **0** rules — there is no `cata/strict_lex2.tex` and no generator value for it |
...
`strict_lex2` is [`lex2`](lex2.md) with `lex_less` in place of `lex_lesseq` on each adjacent row
...
**The naming question, stated and left open.** `strict_lex2` and `lex2_strict` look like two spellings of
...
`tools/data/minizinc-2.10.1-globals.txt:117` carries the *name* only. `decomps/strict_lex2.md` is four lines and is a pointer: "See `decomps/lex2.md` … Same shape, `lex_less` instead of `lex_lesseq` per row pair." It gives no
...
**Generator value:** none. No value in `explenation generator.ml` encodes `strict_lex2`, and none of
the fifteen `explainall` calls (lines 879-893) mentions it.
**Emitted by:** nothing.
**Spec:** `decomps/strict_lex2.md` → `decomps/lex2.md` (which covers `lex2`, `strict_lex2` and `lex2_strict`
...
the input format, so the generator was never asked. There is no `cata/strict_lex2.tex` and therefore
...
**None.** There is no `cata/strict_lex2.tex` (`ls cata/` → 16 files, none of this name; 2026-09-21).
...
- `ls cata/` → 16 `.tex` files, none named `strict_lex2.tex`.
...
- `explenation generator.ml` read, not run → `:488`/`:517` (the raising printers),
  `:879-893` (the fifteen `explainall` calls — none is `strict_lex2`).
- `CHRISTMAS_LIST.md:142`, `:106-109` read → the citation, the one-row treatment of all five
  names, the solver legend.
- `decomps/strict_lex2.md`, `decomps/lex2.md`, `decomps/lex_less.md`, `decomps/_shapes.md:44-47,155-175`,
