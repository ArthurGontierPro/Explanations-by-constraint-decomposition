...
> explain. It is not a list of all valid explanations of `count_fn`,
> and no claim of that kind is made anywhere in this catalog.** See
> `catalog/README.md`, "What the catalog claims".

**This entry is a pointer.** `count_fn` is the **functional form** of
[`count`](count.md) and is not a separate constraint. Everything this
method has to say about it is in [`catalog/count.md`](count.md); nothing is duplicated here,
...
| **Last measured** | 2026-09-21, `python3 tools/mzn_coverage.py --rank --json`, `make validate`, `ls cata/`, and reads of `CHRISTMAS_LIST.md:217`, `CHRISTMAS_LIST.md:128` and `catalog/count.md` |

## Constraint

`count_fn(...)` — **not vendored in this repo.** The MiniZinc library is not checked in:
`tools/data/minizinc-2.10.1-globals.txt:43` carries the *name* `count_fn` and nothing else,
...
Read with the `count` row at `CHRISTMAS_LIST.md:128`, that says `count_fn` introduces a
result — one `var int`: how many of the `x[i]` equal `y` — and posts `count` over it.

**Provenance of the name:** `tools/data/minizinc-2.10.1-globals.txt:43`.
...
2. **A pointer is a finding, and an absence is not.** Saying "`count_fn` is `count` under
   another syntax, so its explanation is `count`'s" is a checkable claim. Leaving eleven files
...
3. `find`'s AND/OR traversal, `an`'s DNF flattening and the LaTeX printer are functions of that
   ...
   `cata/count_fn.tex` to contain that `cata/`'s `count` artifact does not already contain.**
...
result were not already determined by the other arguments — but for `count_fn` it is
(the count is a function of `x` and `y`), so the functional form adds no literal and no explanation. *This is reasoning
...
it. This entry supplies the missing route and its reason: **the route of `count_fn` is the route
of `count`**, because it is the same decomposition. `CHRISTMAS_LIST.md:128`'s route cell
for `count` reads, verbatim:
...
their bases are spread across **A**, **B**, **C** and **out of scope**. `count` is
...
no paper explains this constraint. The citation that applies is `count`'s, at
`CHRISTMAS_LIST.md:128`, and it is quoted in [`count.md`](count.md).

**Rule shape:** see [`count.md`](count.md). Nothing about any paper's content is stated
...
**Source: the `count` row, `CHRISTMAS_LIST.md:128`** — not the `*_fn` row, whose solver
...
world: a solver never sees `count_fn`, it sees the `count` the function posts, so the
...
`count_fn` and should not: the decomposition to encode is `count`'s.
**Emitted by:** nothing — `ls cata/` (2026-09-21) lists 16 `.tex` files and none is
`count_fn.tex`.
**Spec:** none — there is no `decomps/count_fn.md`, and the 43 specs under `decomps/` are
named for predicate forms. [`count.md`](count.md) records whether `count` has one.
...
invoked for `count_fn`, so there is no `%% generator diagnostics (W1-T3)` footer to read.
...
`count`, and [`count.md`](count.md)'s **Scope of this entry** section states it, including
the events that came back empty. **This entry adds no scope of its own and claims none.**

## Generated rules

**None under this name**, by design rather than by failure: `grep -o '\\frac' cata/count_fn.tex`
...
name. What **is** established, and is this entry's content, is the resolution itself: `count_fn`
is `count` in functional syntax, the decomposition and therefore the explanation are the same
object, and the argument for that is above with its two qualifications attached.

## Calibration (W3-T5, D-0013)

**Verdict: resolves to [`count.md`](count.md).**

No comparison is stated here and none should be: with no rule generated under this name there is
no premise to place in an implication order, and the published side is `count`'s. Note in
...
Whatever verdict [`count.md`](count.md) reaches is `count_fn`'s verdict, for the same reason
...
| (base's) | whatever [`count.md`](count.md)'s Gaps table lists for `count` applies here unchanged |
...
E-code from it. R2's reading, above: the applicable route is `count`'s, quoted verbatim in
"The `*_fn` decision" from `CHRISTMAS_LIST.md:128`.
Source: `docs/DECOMP_FORMAT_NOTES.md` (consolidated wave-two numbering) for what the numbered
gaps mean; none is attributed to `count_fn` itself.

## How this entry was produced

- `python3 tools/mzn_coverage.py --rank --json` (2026-09-21) → `count_fn` in `- unclassified`,
  no E-codes parsed, `CHRISTMAS_LIST.md` line 217; and `count` in `A no-literature + solver-decomposes`.
...
- `CHRISTMAS_LIST.md:128` read → the `count` row: the solver cells in the table above and
...
- `make validate` (run 2026-09-21, output redirected to a file then grepped, per `CLAUDE.md`
  ...
  that run mentions `count_fn`, which is the measurement behind the **Validator** row.
