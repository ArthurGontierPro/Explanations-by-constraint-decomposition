...
> explain. It is not a list of all valid explanations of `lex_chain_lesseq_orbitope`, and no
...
| **Generated** | **0** rules — there is no `cata/lex_chain_lesseq_orbitope.tex` and no generator value for it |
...
So this is [`lex_chain_lesseq`](lex_chain_lesseq.md) applied twice to the same matrix, along two
...
`lex_chain_lesseq_orbitope(...)` — **the argument list is not established here and this entry
will not invent one.**

A matrix is in orbitope form under a non-decreasing lexicographic ordering: the standard symmetry-breaking shape for a matrix with both row and column symmetry, combining row-wise and column-wise `lex_lesseq` chains on the same matrix (`decomps/orbitope.md:3-6`).

**Provenance of the signature:** not vendored in this repo, and **not recoverable from it**.
`tools/data/minizinc-2.10.1-globals.txt:84` carries the *name* only; this checkout holds
...
> `decomps/lex_chain_lesseq_orbitope.md`.` That probe is an exact-filename `os.path.isfile` and
...
is [`lex_chain_lesseq`](lex_chain_lesseq.md)'s, unchanged. `tied_i` is a **genuine
...
the input format, so the generator was never asked. There is no `cata/lex_chain_lesseq_orbitope.tex`
...
**None.** There is no `cata/lex_chain_lesseq_orbitope.tex` (`ls cata/` → 16 files, none of this
...
- `ls cata/` → 16 `.tex` files, none named `lex_chain_lesseq_orbitope.tex`.
...
- `tools/data/minizinc-2.10.1-globals.txt:84` read → the name.
