...
> explain. It is not a list of all valid explanations of `steiner`, and no claim
...
| **Validator** | out of scope — no `cata/steiner.tex` exists for `make validate` to check |
...
`steiner` — no MiniZinc signature is vendored in this repo.

**Provenance of the name:** `tools/data/minizinc-2.10.1-globals.txt` lists `steiner` as a
...
`steiner`, and none is written here (`catalog/TEMPLATE.md`: never state a rule shape from
memory).
...
**None.** There is no `decomps/steiner.md` and no `cata/steiner.tex`; no decomposition in
this project's format has been written for `steiner`.
...
**Is that classification right for `steiner`?** Yes. Every constraint in this group has a **native** Chuffed propagator built on a dedicated graph algorithm (see Solver support), and the published explanations that exist (Francis & Stuckey for `circuit`/`subcircuit`) are reachability arguments over the run-time graph, not over a finite unrolled Boolean formula. This is not a section-filter artefact: the route cell itself says `E6`, and E6 is one of the three categories `docs/ROADMAP.md:106` names explicitly.
...
None. No `cata/steiner.tex` exists, so there is nothing to render.

## Status

**`nothing generated — blocked on E6`**

No decomposition has been attempted, encoded or generated for `steiner`. This is a scope
...
- `python3 tools/mzn_coverage.py --rank` (run 2026-09-21) → `steiner` under tier `- out of scope`,
...
- `os.path.isfile("decomps/steiner.md")` → False. `os.path.isfile("cata/steiner.tex")` → False.
