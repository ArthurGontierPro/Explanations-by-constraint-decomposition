...
| **Generated** | **7** rules in `cata/arg_min.tex` — **new 2026-09-22** (session A-1), from generator value `argmin`. The decomposition is **authored here**: there is no `decomps/arg_min.md` |
| **Validator** | **not covered — the validator cannot see this file.** `make validate` (run 2026-09-22) names no `arg_min` entry, in scope or out: `validator.ml`'s lists are hardcoded and it never scans `cata/`. That is **W1-T18** |
...
| **Last measured** | 2026-09-22 (session A-1), `make check`, `make validate`, `grep -o '\frac' cata/arg_min.tex \| wc -l`, and this session's own exhaustive assignment sweep under two tie-breaking readings, with per-premise droppability. The Tier row is the 2026-09-21 `mzn_coverage.py` run, unre-run |
...
**Generator value:** `argmin`, `explenation generator.ml:1301`.
**Emitted by:** `explainall [xbc;mbc;i] argmin "cata/arg_min.tex"`, line **1653**. All three seeds already
existed: `xbc` (2020), `mbc` ([`maximum`](maximum.md)'s scalar bound, 2026-09-22) and `i`
([`element`](element.md)'s index variable, 2020).

**There is still no `decomps/arg_min.md`.** This decomposition was **authored in the generator**,
...
| 2 | `rule3` | `B2_t ⇔ ⋀_{i} B1_{i,t}` |
| 3 | `rule1` | `O ≥ t ⇔ B2_t` (BC) |
| 4 | `rule1` | `I = i ⇔ B3_i` (AC) |
| 5 | `rule1` | `O ≥ t ⇔ B4_t` (BC) — **a second name for the same literal** |
| 6 | `rule1` | `X_i ≥ t ⇔ B5_{i,t}` (BC) — **likewise** |
| 7 | `rule4` | the clause: `I = i ∧ O ≥ t → X_i ≥ t` (with the signs the min direction needs) |

Constraints 1–3 are [`minimum`](minimum.md)'s channel verbatim. Constraint 7 is
...
MiniZinc's `arg_min` returns the **first** minimal index; this decomposition says only that `I` is
**a** minimal index. The decomposition is therefore a **relaxation** of the constraint, and a
...
**Why it was filed out of scope.** `tools/mzn_coverage.py --rank` puts `arg_min`
...
decomposition can conclude that the argmin *is* a given index — only that it is not. This is
...
`grep -o '\frac' cata/arg_min.tex | wc -l` → **7**, measured 2026-09-22.

| # | rule | verdict (second instrument; **not** `make validate`) |
|---|---|---|
| 1 | `O ≥ t`  ⊢  `X_i ≥ t` | **SOUND**, **MINIMAL** |
| 2 | `∀i'≠i: X_{i'} ≥ t`, `O < t`  ⊢  `X_i < t` | **SOUND**, **MINIMAL** |
| 3 | `O < t`, `I = i`  ⊢  `X_i < t` | **SOUND**, **MINIMAL** |
| 4 | `∀i: X_i ≥ t`  ⊢  `O ≥ t` | **SOUND**, **MINIMAL** |
| 5 | `X_i ≥ t`, `I = i`  ⊢  `O ≥ t` | **SOUND**, **MINIMAL** |
| 6 | `∃i: X_i < t`  ⊢  `O < t` | **SOUND**, **MINIMAL** |
| 7 | `X_i ≥ t`, `O < t`  ⊢  `I ≠ i` | **SOUND**, **MINIMAL** |

Rule 7 is the one that does real work: a variable shown to be at or above a threshold the bound is already below cannot be the argmin.

### How the verdicts were obtained

**This session's own sweep, not `make validate`.** Exhaustive enumeration of every
`X ∈ [1,m]^n` with `O = min_i X_i` and `I` an index achieving it, for every
...
file order, any-minimal-index reading, at `n,m ≤ 4`: **4101 / 349 / 1654 / 1133 / 1133 / 1654 / 2262**; at `n,m ≤ 5`: **62487 / 4158 / 28200 / 13287 / 13287 / 28200 / 53196**. Under the
first-index reading at `n,m ≤ 5`: **37329 / 4158 / 18204 / 7995 / 7995 / 18204 / 37950**. **All seven minimal** over the range.

**At `n = 1`** firings are **35 / 20 / 20 / 35 / 35 / 20 / 0** and no rule fails. Rule 7 fires **0** times there *for a
reason*: with one variable, `X_1` **is** the bound, so its premise cannot hold. Rules 2, 3 and 5 have a
...
- `grep -o '\frac' cata/arg_min.tex | wc -l` → **7**.
