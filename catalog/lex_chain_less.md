...
> explain. It is not a list of all valid explanations of `lex_chain_less`, and no claim of
...
| **Generated** | **0** rules — there is no `cata/lex_chain_less.tex` and no generator value for it |
...
`lex_chain_less` is [`lex_less`](lex_less.md) chained: the same accumulated-state chain, applied to every adjacent pair of a matrix's vectors instead of to one pair. **Everything about the chain, the matrix
...
**What the strictness costs: nothing new.** Against
[`lex_chain_lesseq`](lex_chain_lesseq.md) this entry differs in one disjunct of one `rule4`
clause — the per-position disjunction keeps the "strict somewhere" requirement, so a fully tied
pair is a violation rather than accepting. `decomps/_shapes.md:52-64`, convention 1, is why that
is not a shape difference: `{rule3, rule4}` is one schema family because `Decomp_devent` carries
a free sign bit on every summand, and relaxing or keeping one disjunct is strictly less than a
De Morgan flip. Both are **S5 + M-row**.

## Constraint

`lex_chain_less(array[int,int] of var int: x)`

Each adjacent pair of `x`'s vectors is lexicographically **strictly** increasing: every pair satisfies [`lex_less`](lex_less.md). A chained symmetry-breaking constraint.

**Provenance of the signature:** not vendored in this repo.
`tools/data/minizinc-2.10.1-globals.txt:82` carries the *name* only; this checkout holds no
...
> `decomps/lex_chain_less.md`.` That probe is an exact-filename `os.path.isfile` and it misses a
...
[`lex_less`](lex_less.md)'s, unchanged, and the whole block is then replicated over the
...
the input format, so the generator was never asked. There is no `cata/lex_chain_less.tex` and
...
**None.** There is no `cata/lex_chain_less.tex` (`ls cata/` → 16 files, none of this name;
...
The strictness changes which `rule4` clause is written, not which literals exist. G3 is the binding gap and is not this constraint's alone
...
- `ls cata/` → 16 `.tex` files, none named `lex_chain_less.tex`.
...
- `tools/data/minizinc-2.10.1-globals.txt:82` read → the name.
