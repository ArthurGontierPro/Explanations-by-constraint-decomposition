...
> explain. It is not a list of all valid explanations of `range_fn`,
> and no claim of that kind is made anywhere in this catalog.** See
> `catalog/README.md`, "What the catalog claims".

**This entry is a pointer.** `range_fn` is the **functional form** of
[`range`](range.md) and is not a separate constraint. Everything this
method has to say about it is in [`catalog/range.md`](range.md); nothing is duplicated here,
...
| | |
|---|---|
| **Tier** | **unclassified** (`- unclassified`) — `python3 tools/mzn_coverage.py --rank`, 2026-09-21. **This is an artifact of a blank cell, not a property of the constraint** — see below. The base `range` ranks **out of scope** (`- out of scope`) |
| **Status** | **resolves to [`range.md`](range.md)**, whose Status row reads `nothing generated — blocked on G8` (read off that file, 2026-09-21). **No status-legend value is asserted for `range_fn` itself**, and this entry generates nothing of its own |
| **Generated** | **0** — there is no `cata/range_fn.tex`, and there should not be one. The rules for this constraint are the rules in [`range.md`](range.md) |
...
| **Calibration** | **resolves to [`range.md`](range.md)**. `CHRISTMAS_LIST.md:217` records the literature column for this row as `—`; that is a statement about the *row*, not a finding that no paper explains range, so `no published rule exists` is **not** claimed here |
| **Last measured** | 2026-09-21, `python3 tools/mzn_coverage.py --rank --json`, `make validate`, `ls cata/`, and reads of `CHRISTMAS_LIST.md:217`, `CHRISTMAS_LIST.md:206` and `catalog/range.md` |

## Constraint

`range_fn(...)` — **not vendored in this repo.** The MiniZinc library is not checked in:
`tools/data/minizinc-2.10.1-globals.txt:103` carries the *name* `range_fn` and nothing else,
...
Read with the `range` row at `CHRISTMAS_LIST.md:206`, that says `range_fn` introduces a
result — one `var set of int`: the image of the index set `s` under `x` — and posts `range` over it.

**Provenance of the name:** `tools/data/minizinc-2.10.1-globals.txt:103`.
...
2. **A pointer is a finding, and an absence is not.** Saying "`range_fn` is `range` under
   another syntax, so its explanation is `range`'s" is a checkable claim. Leaving eleven files
...
3. `find`'s AND/OR traversal, `an`'s DNF flattening and the LaTeX printer are functions of that
   ...
   `cata/range_fn.tex` to contain that `cata/`'s `range` artifact does not already contain.**
...
result were not already determined by the other arguments — but for `range_fn` it is
(the image set is a function of `x` and `s`), so the functional form adds no literal and no explanation. *This is reasoning
...
it. This entry supplies the missing route and its reason: **the route of `range_fn` is the route
of `range`**, because it is the same decomposition. `CHRISTMAS_LIST.md:206`'s route cell
for `range` reads, verbatim:
...
their bases are spread across **A**, **B**, **C** and **out of scope**. `range` is
...
no paper explains this constraint. The citation that applies is `range`'s, at
`CHRISTMAS_LIST.md:206`, and it is quoted in [`range.md`](range.md).

**Rule shape:** see [`range.md`](range.md). Nothing about any paper's content is stated
...
**Source: the `range` row, `CHRISTMAS_LIST.md:206`** — not the `*_fn` row, whose solver
...
world: a solver never sees `range_fn`, it sees the `range` the function posts, so the
...
`range_fn` and should not: the decomposition to encode is `range`'s.
**Emitted by:** nothing — `ls cata/` (2026-09-21) lists 16 `.tex` files and none is
`range_fn.tex`.
**Spec:** none — there is no `decomps/range_fn.md`, and the 43 specs under `decomps/` are
named for predicate forms. [`range.md`](range.md) records whether `range` has one.
...
invoked for `range_fn`, so there is no `%% generator diagnostics (W1-T3)` footer to read.
...
`range`, and [`range.md`](range.md)'s **Scope of this entry** section states it, including
the events that came back empty. **This entry adds no scope of its own and claims none.**

## Generated rules

**None under this name**, by design rather than by failure: `grep -o '\\frac' cata/range_fn.tex`
...
name. What **is** established, and is this entry's content, is the resolution itself: `range_fn`
is `range` in functional syntax, the decomposition and therefore the explanation are the same
object, and the argument for that is above with its two qualifications attached.

## Calibration (W3-T5, D-0013)

**Verdict: resolves to [`range.md`](range.md).**

No comparison is stated here and none should be: with no rule generated under this name there is
no premise to place in an implication order, and the published side is `range`'s. Note in
...
Whatever verdict [`range.md`](range.md) reaches is `range_fn`'s verdict, for the same reason
...
| (base's) | whatever [`range.md`](range.md)'s Gaps table lists for `range` applies here unchanged |
...
E-code from it. R2's reading, above: the applicable route is `range`'s, quoted verbatim in
...
gaps mean; none is attributed to `range_fn` itself.

## How this entry was produced

- `python3 tools/mzn_coverage.py --rank --json` (2026-09-21) → `range_fn` in `- unclassified`,
  no E-codes parsed, `CHRISTMAS_LIST.md` line 217; and `range` in `- out of scope`.
...
- `CHRISTMAS_LIST.md:206` read → the `range` row: the solver cells in the table above and
  the route cell quoted verbatim in "The `*_fn` decision".
- `CHRISTMAS_LIST.md:106-109` read → the solver-column legend.
- `tools/data/minizinc-2.10.1-globals.txt:103` read → the name, and that the snapshot carries
  names only.
- `catalog/range.md` read, not run → the base entry's Status row, quoted above as
  `nothing generated — blocked on G8`, and the fact that it carries the rules, scope and calibration this entry defers to.
- `ls cata/` → 16 `.tex` files, none named `range_fn.tex`.
- `make validate` (run 2026-09-21, output redirected to a file then grepped, per `CLAUDE.md`
  ...
  that run mentions `range_fn`, which is the measurement behind the **Validator** row.
