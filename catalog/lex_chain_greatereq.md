...
> explain. It is not a list of all valid explanations of `lex_chain_greatereq`, and no claim of
...
| **Generated** | **0** rules — there is no `cata/lex_chain_greatereq.tex` and no generator value for it |
...
`lex_chain_greatereq` is [`lex_chain_lesseq`](lex_chain_lesseq.md) with its operands exchanged: MiniZinc defines `lex_greatereq(x,y)` as `lex_lesseq(y,x)` (`CHRISTMAS_LIST.md:143`), and the chain inherits that swap pair by pair. **Everything about the chain, the matrix
...
**Two reductions, both already carried elsewhere.** The argument swap is
[`lex_greatereq`](lex_greatereq.md)'s, stated with its citation there; the relaxed comparator is
[`lex_lesseq`](lex_lesseq.md)'s, where a fully tied pair is accepting rather than a violation.
Neither is a shape difference (`decomps/_shapes.md:52-64`, convention 1), and this entry is
**S5 + M-row** like every other name on the row.

## Constraint

`lex_chain_greatereq(array[int,int] of var int: x)`

Each adjacent pair of `x`'s vectors is lexicographically non-increasing: every pair satisfies [`lex_greatereq`](lex_greatereq.md), which is [`lex_lesseq`](lex_lesseq.md) with its arguments exchanged.

**Provenance of the signature:** not vendored in this repo.
`tools/data/minizinc-2.10.1-globals.txt:80` carries the *name* only; this checkout holds no
...
> `decomps/lex_chain_greatereq.md`.` That probe is an exact-filename `os.path.isfile` and it misses a
...
[`lex_chain_lesseq`](lex_chain_lesseq.md)'s, unchanged, and the whole block is then replicated over the
...
the input format, so the generator was never asked. There is no `cata/lex_chain_greatereq.tex` and
...
**None.** There is no `cata/lex_chain_greatereq.tex` (`ls cata/` → 16 files, none of this name;
...
Neither the swap nor the relaxed comparator moves the gap: G3 is the binding gap and is not this constraint's alone
...
- `ls cata/` → 16 `.tex` files, none named `lex_chain_greatereq.tex`.
...
- `tools/data/minizinc-2.10.1-globals.txt:80` read → the name.
