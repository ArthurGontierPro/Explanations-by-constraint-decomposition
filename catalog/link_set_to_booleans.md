...
> explain. It is not a list of all valid explanations of `link_set_to_booleans`, and no claim
...
| **Generated** | no generator entry — there is no `cata/link_set_to_booleans.tex` |
| **Validator** | out of scope — no `cata/link_set_to_booleans.tex` exists for `make validate` to check |
...
`link_set_to_booleans` — no MiniZinc signature is vendored in this repo.

**Provenance of the name:** `tools/data/minizinc-2.10.1-globals.txt` lists `link_set_to_booleans` as a
...
`link_set_to_booleans`, and none is written here (`catalog/TEMPLATE.md`: never state a rule shape from
memory).
...
**None.** There is no `decomps/link_set_to_booleans.md` and no `cata/link_set_to_booleans.tex`; no decomposition in
this project's format has been written for `link_set_to_booleans`.
...
**Is that classification right for `link_set_to_booleans`?** Correctly filed, but with a sharper point than the rest of this group: `CHRISTMAS_LIST.md:200` states plainly that `link_set_to_booleans` **is** the E5 mechanism, i.e. `b[i,v] <-> v in S_i` is not merely *needed for* this constraint, it is *this constraint's own definition*. So `link_set_to_booleans` is not blocked on E5 the way its siblings are — implementing E5 and explaining `link_set_to_booleans` are the same piece of work.

**What would bring it in scope.** Not "E5 lands, then this constraint becomes explainable" but "explaining this constraint's own channel *is* landing E5" — this should be the first target when E5 work starts, not a downstream beneficiary of it.

## Generated rules

None. No `cata/link_set_to_booleans.tex` exists, so there is nothing to render.

## Status

**`nothing generated — blocked on E5`**

No decomposition has been attempted, encoded or generated for `link_set_to_booleans`. This is a scope
...
- `python3 tools/mzn_coverage.py --rank` (run 2026-09-21) → `link_set_to_booleans` under tier `- out of scope`,
...
- `os.path.isfile("decomps/link_set_to_booleans.md")` → False. `os.path.isfile("cata/link_set_to_booleans.tex")` → False.
