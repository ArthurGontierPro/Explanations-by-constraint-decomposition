...
> explain. It is not a list of all valid explanations of `diffn_nonstrict_k`, and no claim
...
| **Generated** | no generator entry — there is no `cata/diffn_nonstrict_k.tex` |
| **Validator** | not run — no `cata/diffn_nonstrict_k.tex` exists yet |
...
`diffn_nonstrict_k` — no MiniZinc signature is vendored in this repo.
`tools/data/minizinc-2.10.1-globals.txt` lists `diffn_nonstrict_k` as a global of MiniZinc 2.10.1
...
No paper is sourced in `catalog/_literature/` for `diffn_nonstrict_k`. `CHRISTMAS_LIST.md:51` is the
...
published explanation for `diffn_nonstrict_k` itself.
...
**None yet.** There is no `decomps/diffn_nonstrict_k.md` and no `cata/diffn_nonstrict_k.tex`. What it would take is
below.
...
needs, not by the section it is filed under, and `diffn_nonstrict_k`'s mechanism — variable-vs-variable
...
variables. `diffn_nonstrict_k`'s non-overlap disjunction is exactly this wall.
...
ask for", item 2, "biggest single unlock"). Second, someone writes `decomps/diffn_nonstrict_k.md`
...
misclassification — `diffn_nonstrict_k`'s own route cell says `E2`, not E5/E6/E7, and `diffn_nonstrict_k` is named
...
35→28). `diffn_nonstrict_k` is filed here under its corrected tier as a result — the classification was
checked, not assumed.

## Generated rules

None. No `cata/diffn_nonstrict_k.tex` exists.
...
|---|---|
| `G3` | only variable-vs-domain-value comparisons exist in the event type, never variable-vs-variable; `diffn_nonstrict_k`'s non-overlap disjunction needs `s_i+d_i \le s_j`-style atoms (`docs/DECOMP_FORMAT_NOTES.md:34-41`) |
...
- `python3 tools/mzn_coverage.py --rank` (re-run 2026-09-21, post-fix) → `diffn_nonstrict_k` under tier
...
- `CHRISTMAS_LIST.md:227-238` read → the "What to actually ask for" priority list naming
  `diffn_nonstrict_k` beside `disjunctive`/`maximum`/`minimum`/`table`/`regular`.
...
- `os.path.isfile("decomps/diffn_nonstrict_k.md")` → False. `os.path.isfile("cata/diffn_nonstrict_k.tex")` → False.
- Nothing was compiled, run or validated. No rules exist yet for `diffn_nonstrict_k`; this entry states
