...
[`decreasing`](decreasing.md) is **validated: sound and minimal at n,m ≤ 4**, 2 rules, 2
...
constraint, so there is no `cata/strictly_decreasing.tex` and no
...
of `cata/decreasing.tex`: two events, 2 candidates each, 1 rule each, with
...
**None.** There is no `cata/strictly_decreasing.tex` (`ls cata/` → 16 files, none of this
name; 2026-09-21). Nothing is rendered here and nothing is claimed.

**The base's rules are not reproduced here** — they are [`decreasing`](decreasing.md)'s
