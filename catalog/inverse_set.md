...
> explain. It is not a list of all valid explanations of `inverse_set`, and no claim
...
| **Generated** | no generator entry — there is no `cata/inverse_set.tex` |
| **Validator** | out of scope — no `cata/inverse_set.tex` exists for `make validate` to check |
...
`inverse_set` — no MiniZinc signature is vendored in this repo.

**Provenance of the name:** `tools/data/minizinc-2.10.1-globals.txt` lists `inverse_set` as a
...
`inverse_set`, and none is written here (`catalog/TEMPLATE.md`: never state a rule shape from
memory).
...
**None.** There is no `decomps/inverse_set.md` and no `cata/inverse_set.tex`; no decomposition in
this project's format has been written for `inverse_set`.
...
**Is that classification right for `inverse_set`?** Yes, correctly filed — set-valued `inverse`, needs the same membership matrix as `disjoint`/`partition_set`/`sum_set` on the same source line.

**What would bring it in scope.** E5, the membership matrix.

## Generated rules

None. No `cata/inverse_set.tex` exists, so there is nothing to render.

## Status

**`nothing generated — blocked on E5`**

No decomposition has been attempted, encoded or generated for `inverse_set`. This is a scope
...
- `python3 tools/mzn_coverage.py --rank` (run 2026-09-21) → `inverse_set` under tier `- out of scope`,
...
- `os.path.isfile("decomps/inverse_set.md")` → False. `os.path.isfile("cata/inverse_set.tex")` → False.
