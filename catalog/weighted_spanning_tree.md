...
> explain. It is not a list of all valid explanations of `weighted_spanning_tree`, and no claim
...
| **Generated** | no generator entry — there is no `cata/weighted_spanning_tree.tex` |
| **Validator** | out of scope — no `cata/weighted_spanning_tree.tex` exists for `make validate` to check |
...
`weighted_spanning_tree` — no MiniZinc signature is vendored in this repo.

**Provenance of the name:** `tools/data/minizinc-2.10.1-globals.txt` lists `weighted_spanning_tree` as a
...
`weighted_spanning_tree`, and none is written here (`catalog/TEMPLATE.md`: never state a rule shape from
memory).
...
**None.** There is no `decomps/weighted_spanning_tree.md` and no `cata/weighted_spanning_tree.tex`; no decomposition in
this project's format has been written for `weighted_spanning_tree`.
...
None. No `cata/weighted_spanning_tree.tex` exists, so there is nothing to render.

## Status

**`nothing generated — blocked on E6`**

No decomposition has been attempted, encoded or generated for `weighted_spanning_tree`. This is a scope
...
- `python3 tools/mzn_coverage.py --rank` (run 2026-09-21) → `weighted_spanning_tree` under tier `- out of scope`,
...
- `os.path.isfile("decomps/weighted_spanning_tree.md")` → False. `os.path.isfile("cata/weighted_spanning_tree.tex")` → False.
