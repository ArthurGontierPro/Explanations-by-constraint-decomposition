...
> explain. It is not a list of all valid explanations of `global_cardinality_fn`,
> and no claim of that kind is made anywhere in this catalog.** See
> `catalog/README.md`, "What the catalog claims".

**This entry is a pointer.** `global_cardinality_fn` is the **functional form** of
[`global_cardinality`](gcc.md) and is not a separate constraint. Everything this
method has to say about it is in [`catalog/gcc.md`](gcc.md); nothing is duplicated here,
...
| **Last measured** | 2026-09-21, `python3 tools/mzn_coverage.py --rank --json`, `make validate`, `ls cata/`, and reads of `CHRISTMAS_LIST.md:217`, `CHRISTMAS_LIST.md:127` and `catalog/gcc.md` |

## Constraint

`global_cardinality_fn(...)` — **not vendored in this repo.** The MiniZinc library is not checked in:
`tools/data/minizinc-2.10.1-globals.txt:67` carries the *name* `global_cardinality_fn` and nothing else,
...
Read with the `global_cardinality` row at `CHRISTMAS_LIST.md:127`, that says `global_cardinality_fn` introduces a
result — an `array of var int`: the count of each value of `cover` — and posts `global_cardinality` over it.

**Provenance of the name:** `tools/data/minizinc-2.10.1-globals.txt:67`.
...
2. **A pointer is a finding, and an absence is not.** Saying "`global_cardinality_fn` is `global_cardinality` under
   another syntax, so its explanation is `global_cardinality`'s" is a checkable claim. Leaving eleven files
...
result were not already determined by the other arguments — but for `global_cardinality_fn` it is
...
it. This entry supplies the missing route and its reason: **the route of `global_cardinality_fn` is the route
of `global_cardinality`**, because it is the same decomposition. `CHRISTMAS_LIST.md:127`'s route cell
for `global_cardinality` reads, verbatim:
...
their bases are spread across **A**, **B**, **C** and **out of scope**. `global_cardinality` is
...
no paper explains this constraint. The citation that applies is `global_cardinality`'s, at
`CHRISTMAS_LIST.md:127`, and it is quoted in [`gcc.md`](gcc.md).

**Rule shape:** see [`gcc.md`](gcc.md). Nothing about any paper's content is stated
...
**Source: the `global_cardinality` row, `CHRISTMAS_LIST.md:127`** — not the `*_fn` row, whose solver
...
world: a solver never sees `global_cardinality_fn`, it sees the `global_cardinality` the function posts, so the
...
`global_cardinality_fn` and should not: the decomposition to encode is `global_cardinality`'s.
**Emitted by:** nothing — `ls cata/` (2026-09-21) lists 16 `.tex` files and none is
`global_cardinality_fn.tex`.
**Spec:** none — there is no `decomps/global_cardinality_fn.md`, and the 43 specs under `decomps/` are
named for predicate forms. [`gcc.md`](gcc.md) records whether `global_cardinality` has one.
...
invoked for `global_cardinality_fn`, so there is no `%% generator diagnostics (W1-T3)` footer to read.
...
`global_cardinality`, and [`gcc.md`](gcc.md)'s **Scope of this entry** section states it, including
the events that came back empty. **This entry adds no scope of its own and claims none.**

## Generated rules

**None under this name**, by design rather than by failure: `grep -o '\\frac' cata/global_cardinality_fn.tex`
...
name. What **is** established, and is this entry's content, is the resolution itself: `global_cardinality_fn`
is `global_cardinality` in functional syntax, the decomposition and therefore the explanation are the same
object, and the argument for that is above with its two qualifications attached.

## Calibration (W3-T5, D-0013)

**Verdict: resolves to [`gcc.md`](gcc.md).**

No comparison is stated here and none should be: with no rule generated under this name there is
no premise to place in an implication order, and the published side is `global_cardinality`'s. Note in
...
Whatever verdict [`gcc.md`](gcc.md) reaches is `global_cardinality_fn`'s verdict, for the same reason
...
| (base's) | whatever [`gcc.md`](gcc.md)'s Gaps table lists for `global_cardinality` applies here unchanged |
...
E-code from it. R2's reading, above: the applicable route is `global_cardinality`'s, quoted verbatim in
...
gaps mean; none is attributed to `global_cardinality_fn` itself.

## How this entry was produced

- `python3 tools/mzn_coverage.py --rank --json` (2026-09-21) → `global_cardinality_fn` in `- unclassified`,
  no E-codes parsed, `CHRISTMAS_LIST.md` line 217; and `global_cardinality` in `C literature + solver-decomposes`.
...
- `CHRISTMAS_LIST.md:127` read → the `global_cardinality` row: the solver cells in the table above and
