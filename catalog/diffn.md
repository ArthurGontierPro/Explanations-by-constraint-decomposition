...
| **Validator** | not run — no `cata/diffn.tex` exists yet |
...
No paper is sourced in `catalog/_literature/` for `diffn`. `CHRISTMAS_LIST.md:51` is the
...
published explanation for `diffn` itself.
...
**None yet.** There is no `decomps/diffn.md` and no `cata/diffn.tex`. What it would take is
below.
...
needs, not by the section it is filed under, and `diffn`'s mechanism — variable-vs-variable
...
variables. `diffn`'s non-overlap disjunction is exactly this wall.
...
ask for", item 2, "biggest single unlock"). Second, someone writes `decomps/diffn.md`
...
misclassification — `diffn`'s own route cell says `E2`, not E5/E6/E7, and `diffn` is named
...
35→28). `diffn` is filed here under its corrected tier as a result — the classification was
checked, not assumed.
...
|---|---|
| `G3` | only variable-vs-domain-value comparisons exist in the event type, never variable-vs-variable; `diffn`'s non-overlap disjunction needs `s_i+d_i \le s_j`-style atoms (`docs/DECOMP_FORMAT_NOTES.md:34-41`) |
...
- `python3 tools/mzn_coverage.py --rank` (re-run 2026-09-21, post-fix) → `diffn` under tier
...
- `CHRISTMAS_LIST.md:227-238` read → the "What to actually ask for" priority list naming
  `diffn` beside `disjunctive`/`maximum`/`minimum`/`table`/`regular`.
...
- Nothing was compiled, run or validated. No rules exist yet for `diffn`; this entry states
