...
> explain. It is not a list of all valid explanations of `sort_fn`,
> and no claim of that kind is made anywhere in this catalog.** See
> `catalog/README.md`, "What the catalog claims".

**This entry is a pointer.** `sort_fn` is the **functional form** of
[`sort`](sort.md) and is not a separate constraint. Everything this
method has to say about it is in [`catalog/sort.md`](sort.md); nothing is duplicated here,
...
| | |
|---|---|
| **Tier** | **unclassified** (`- unclassified`) — `python3 tools/mzn_coverage.py --rank`, 2026-09-21. **This is an artifact of a blank cell, not a property of the constraint** — see below. The base `sort` ranks **A** (`A no-literature + solver-decomposes`) |
| **Status** | **resolves to [`sort.md`](sort.md)**, whose Status row reads `encodable today, not encoded` (re-read 2026-09-22; it read `not reviewed` when this entry was written, before the base was reviewed). **No status-legend value is asserted for `sort_fn` itself**, and this entry generates nothing of its own |
| **Generated** | **0** — there is no `cata/sort_fn.tex`, and there should not be one. The rules for this constraint are the rules in [`sort.md`](sort.md) |
...
| **Calibration** | **resolves to [`sort.md`](sort.md)**. `CHRISTMAS_LIST.md:217` records the literature column for this row as `—`; that is a statement about the *row*, not a finding that no paper explains sort, so `no published rule exists` is **not** claimed here |
| **Last measured** | 2026-09-21, `python3 tools/mzn_coverage.py --rank --json`, `make validate`, `ls cata/`, and reads of `CHRISTMAS_LIST.md:217`, `CHRISTMAS_LIST.md:198` and `catalog/sort.md` |

## Constraint

`sort_fn(...)` — **not vendored in this repo.** The MiniZinc library is not checked in:
`tools/data/minizinc-2.10.1-globals.txt:114` carries the *name* `sort_fn` and nothing else,
...
Read with the `sort` row at `CHRISTMAS_LIST.md:198`, that says `sort_fn` introduces a
result — an `array of var int`: the multiset of `x` in non-decreasing order — and posts `sort` over it.

**Provenance of the name:** `tools/data/minizinc-2.10.1-globals.txt:114`.
...
2. **A pointer is a finding, and an absence is not.** Saying "`sort_fn` is `sort` under
   another syntax, so its explanation is `sort`'s" is a checkable claim. Leaving eleven files
...
3. `find`'s AND/OR traversal, `an`'s DNF flattening and the LaTeX printer are functions of that
   ...
   `cata/sort_fn.tex` to contain that `cata/`'s `sort` artifact does not already contain.**
...
result were not already determined by the other arguments — but for `sort_fn` it is
(the sorted array is a function of `x`), so the functional form adds no literal and no explanation. *This is reasoning
...
it. This entry supplies the missing route and its reason: **the route of `sort_fn` is the route
of `sort`**, because it is the same decomposition. `CHRISTMAS_LIST.md:198`'s route cell
for `sort` reads, verbatim:
...
their bases are spread across **A**, **B**, **C** and **out of scope**. `sort` is
...
no paper explains this constraint. The citation that applies is `sort`'s, at
`CHRISTMAS_LIST.md:198`, and it is quoted in [`sort.md`](sort.md).

**Rule shape:** see [`sort.md`](sort.md). Nothing about any paper's content is stated
...
**Source: the `sort` row, `CHRISTMAS_LIST.md:198`** — not the `*_fn` row, whose solver
...
world: a solver never sees `sort_fn`, it sees the `sort` the function posts, so the
...
`sort_fn` and should not: the decomposition to encode is `sort`'s.
**Emitted by:** nothing — `ls cata/` (2026-09-21) lists 16 `.tex` files and none is
`sort_fn.tex`.
**Spec:** none — there is no `decomps/sort_fn.md`, and the 43 specs under `decomps/` are
named for predicate forms. [`sort.md`](sort.md) records whether `sort` has one.
...
invoked for `sort_fn`, so there is no `%% generator diagnostics (W1-T3)` footer to read.
...
`sort`, and [`sort.md`](sort.md)'s **Scope of this entry** section states it, including
the events that came back empty. **This entry adds no scope of its own and claims none.**

## Generated rules

**None under this name**, by design rather than by failure: `grep -o '\\frac' cata/sort_fn.tex`
has no file to read. The generated rules for this constraint are rendered in
[`sort.md`](sort.md).

## Status

**resolves to [`sort.md`](sort.md)** — that entry's Status row reads `encodable today, not encoded` (re-read 2026-09-22).
...
name. What **is** established, and is this entry's content, is the resolution itself: `sort_fn`
is `sort` in functional syntax, the decomposition and therefore the explanation are the same
object, and the argument for that is above with its two qualifications attached.

## Calibration (W3-T5, D-0013)

**Verdict: resolves to [`sort.md`](sort.md).**

No comparison is stated here and none should be: with no rule generated under this name there is
no premise to place in an implication order, and the published side is `sort`'s. Note in
...
Whatever verdict [`sort.md`](sort.md) reaches is `sort_fn`'s verdict, for the same reason
...
| (base's) | whatever [`sort.md`](sort.md)'s Gaps table lists for `sort` applies here unchanged |
...
E-code from it. R2's reading, above: the applicable route is `sort`'s, quoted verbatim in
"The `*_fn` decision" from `CHRISTMAS_LIST.md:198`.
Source: `docs/DECOMP_FORMAT_NOTES.md` (consolidated wave-two numbering) for what the numbered
gaps mean; none is attributed to `sort_fn` itself.

## How this entry was produced

- `python3 tools/mzn_coverage.py --rank --json` (2026-09-21) → `sort_fn` in `- unclassified`,
  no E-codes parsed, `CHRISTMAS_LIST.md` line 217; and `sort` in `A no-literature + solver-decomposes`.
...
- `CHRISTMAS_LIST.md:198` read → the `sort` row: the solver cells in the table above and
  the route cell quoted verbatim in "The `*_fn` decision".
- `CHRISTMAS_LIST.md:106-109` read → the solver-column legend.
- `tools/data/minizinc-2.10.1-globals.txt:114` read → the name, and that the snapshot carries
  names only.
- `catalog/sort.md` read, not run → the base entry's Status row, quoted above as
  `encodable today, not encoded`, and the fact that it carries the rules, scope and calibration this entry defers to.
- `ls cata/` → 16 `.tex` files, none named `sort_fn.tex`.
- `make validate` (run 2026-09-21, output redirected to a file then grepped, per `CLAUDE.md`
  ...
  that run mentions `sort_fn`, which is the measurement behind the **Validator** row.
