...
| **Status** | **resolves to [`sort.md`](sort.md)**, whose Status row reads `not reviewed` (read off that file, 2026-09-21). **No status-legend value is asserted for `sort_fn` itself**, and this entry generates nothing of its own |
...
**resolves to [`sort.md`](sort.md)** — that entry's Status row reads `not reviewed`.
