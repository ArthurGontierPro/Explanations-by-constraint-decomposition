# `global_cardinality_closed_fn`

> **This entry lists the rules this method generated for the events it was asked to
> explain. It is not a list of all valid explanations of `global_cardinality_closed_fn`,
> and no claim of that kind is made anywhere in this catalog.** See
> `catalog/README.md`, "What the catalog claims".

**This entry is a pointer.** `global_cardinality_closed_fn` is the **functional form** of
[`global_cardinality_closed`](global_cardinality_closed.md) and is not a separate constraint. Everything this
method has to say about it is in [`catalog/global_cardinality_closed.md`](global_cardinality_closed.md); nothing is duplicated here,
...
| **Last measured** | 2026-09-21, `python3 tools/mzn_coverage.py --rank --json`, `make validate`, `ls cata/`, and reads of `CHRISTMAS_LIST.md:217`, `CHRISTMAS_LIST.md:127` and `catalog/global_cardinality_closed.md` |

## Constraint

`global_cardinality_closed_fn(...)` — **not vendored in this repo.** The MiniZinc library is not checked in:
`tools/data/minizinc-2.10.1-globals.txt:66` carries the *name* `global_cardinality_closed_fn` and nothing else,
...
Read with the `global_cardinality_closed` row at `CHRISTMAS_LIST.md:127`, that says `global_cardinality_closed_fn` introduces a
result — an `array of var int`: the count of each value of `cover` — and posts `global_cardinality_closed` over it.

**Provenance of the name:** `tools/data/minizinc-2.10.1-globals.txt:66`.
...
2. **A pointer is a finding, and an absence is not.** Saying "`global_cardinality_closed_fn` is `global_cardinality_closed` under
   another syntax, so its explanation is `global_cardinality_closed`'s" is a checkable claim. Leaving eleven files
...
3. `find`'s AND/OR traversal, `an`'s DNF flattening and the LaTeX printer are functions of that
   ...
   `cata/global_cardinality_closed_fn.tex` to contain that `cata/`'s `global_cardinality_closed` artifact does not already contain.**
...
result were not already determined by the other arguments — but for `global_cardinality_closed_fn` it is
(each count is a function of `x` and `cover`), so the functional form adds no literal and no explanation. *This is reasoning
...
it. This entry supplies the missing route and its reason: **the route of `global_cardinality_closed_fn` is the route
of `global_cardinality_closed`**, because it is the same decomposition. `CHRISTMAS_LIST.md:127`'s route cell
for `global_cardinality_closed` reads, verbatim:

> `_low_up` is **E0**. Full `global_cardinality` needs **E3** … so it is **E9**, not E3 (D-0011). The flow explanation needs **E4**.
...
their bases are spread across **A**, **B**, **C** and **out of scope**. `global_cardinality_closed` is
**C**. The eleven are not unclassifiable; they are unclassified because a row that
...
no paper explains this constraint. The citation that applies is `global_cardinality_closed`'s, at
`CHRISTMAS_LIST.md:127`, and it is quoted in [`global_cardinality_closed.md`](global_cardinality_closed.md).

**Rule shape:** see [`global_cardinality_closed.md`](global_cardinality_closed.md). Nothing about any paper's content is stated
...
**Source: the `global_cardinality_closed` row, `CHRISTMAS_LIST.md:127`** — not the `*_fn` row, whose solver
...
world: a solver never sees `global_cardinality_closed_fn`, it sees the `global_cardinality_closed` the function posts, so the
...
`global_cardinality_closed_fn` and should not: the decomposition to encode is `global_cardinality_closed`'s.
**Emitted by:** nothing — `ls cata/` (2026-09-21) lists 16 `.tex` files and none is
`global_cardinality_closed_fn.tex`.
**Spec:** none — there is no `decomps/global_cardinality_closed_fn.md`, and the 43 specs under `decomps/` are
named for predicate forms. [`global_cardinality_closed.md`](global_cardinality_closed.md) records whether `global_cardinality_closed` has one.
...
invoked for `global_cardinality_closed_fn`, so there is no `%% generator diagnostics (W1-T3)` footer to read.
...
`global_cardinality_closed`, and [`global_cardinality_closed.md`](global_cardinality_closed.md)'s **Scope of this entry** section states it, including
the events that came back empty. **This entry adds no scope of its own and claims none.**

## Generated rules

**None under this name**, by design rather than by failure: `grep -o '\\frac' cata/global_cardinality_closed_fn.tex`
has no file to read. The generated rules for this constraint are rendered in
[`global_cardinality_closed.md`](global_cardinality_closed.md).
...
name. What **is** established, and is this entry's content, is the resolution itself: `global_cardinality_closed_fn`
is `global_cardinality_closed` in functional syntax, the decomposition and therefore the explanation are the same
object, and the argument for that is above with its two qualifications attached.

## Calibration (W3-T5, D-0013)

**Verdict: resolves to [`global_cardinality_closed.md`](global_cardinality_closed.md).**

No comparison is stated here and none should be: with no rule generated under this name there is
no premise to place in an implication order, and the published side is `global_cardinality_closed`'s. Note in
...
Whatever verdict [`global_cardinality_closed.md`](global_cardinality_closed.md) reaches is `global_cardinality_closed_fn`'s verdict, for the same reason
...
| (base's) | whatever [`global_cardinality_closed.md`](global_cardinality_closed.md)'s Gaps table lists for `global_cardinality_closed` applies here unchanged |
...
E-code from it. R2's reading, above: the applicable route is `global_cardinality_closed`'s, quoted verbatim in
"The `*_fn` decision" from `CHRISTMAS_LIST.md:127`.
Source: `docs/DECOMP_FORMAT_NOTES.md` (consolidated wave-two numbering) for what the numbered
gaps mean; none is attributed to `global_cardinality_closed_fn` itself.

## How this entry was produced

- `python3 tools/mzn_coverage.py --rank --json` (2026-09-21) → `global_cardinality_closed_fn` in `- unclassified`,
  no E-codes parsed, `CHRISTMAS_LIST.md` line 217; and `global_cardinality_closed` in `C literature + solver-decomposes`.
...
- `CHRISTMAS_LIST.md:127` read → the `global_cardinality_closed` row: the solver cells in the table above and
...
- `make validate` (run 2026-09-21, output redirected to a file then grepped, per `CLAUDE.md`
  ...
  that run mentions `global_cardinality_closed_fn`, which is the measurement behind the **Validator** row.
