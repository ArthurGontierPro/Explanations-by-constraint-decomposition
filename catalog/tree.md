...
> explain. It is not a list of all valid explanations of `tree`, and no claim
...
| **Generated** | no generator entry — there is no `cata/tree.tex` |
| **Validator** | out of scope — no `cata/tree.tex` exists for `make validate` to check |
...
`tree` — no MiniZinc signature is vendored in this repo.

**Provenance of the name:** `tools/data/minizinc-2.10.1-globals.txt` lists `tree` as a
...
`tree`, and none is written here (`catalog/TEMPLATE.md`: never state a rule shape from
memory).
...
**None.** There is no `decomps/tree.md` and no `cata/tree.tex`; no decomposition in
this project's format has been written for `tree`.
...
**Is that classification right for `tree`?** Yes. Every constraint in this group has a **native** Chuffed propagator built on a dedicated graph algorithm (see Solver support), and the published explanations that exist (Francis & Stuckey for `circuit`/`subcircuit`) are reachability arguments over the run-time graph, not over a finite unrolled Boolean formula. This is not a section-filter artefact: the route cell itself says `E6`, and E6 is one of the three categories `docs/ROADMAP.md:106` names explicitly.
...
None. No `cata/tree.tex` exists, so there is nothing to render.

## Status

**`nothing generated — blocked on E6`**

No decomposition has been attempted, encoded or generated for `tree`. This is a scope
...
- `python3 tools/mzn_coverage.py --rank` (run 2026-09-21) → `tree` under tier `- out of scope`,
...
- `os.path.isfile("decomps/tree.md")` → False. `os.path.isfile("cata/tree.tex")` → False.
