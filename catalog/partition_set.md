...
> explain. It is not a list of all valid explanations of `partition_set`, and no claim
...
| **Generated** | no generator entry — there is no `cata/partition_set.tex` |
| **Validator** | out of scope — no `cata/partition_set.tex` exists for `make validate` to check |
...
`partition_set` — no MiniZinc signature is vendored in this repo.

**Provenance of the name:** `tools/data/minizinc-2.10.1-globals.txt` lists `partition_set` as a
...
`partition_set`, and none is written here (`catalog/TEMPLATE.md`: never state a rule shape from
memory).
...
**None.** There is no `decomps/partition_set.md` and no `cata/partition_set.tex`; no decomposition in
this project's format has been written for `partition_set`.
...
**Is that classification right for `partition_set`?** Yes, correctly filed alongside `disjoint`/`sum_set`/`inverse_set` on the same source row.

**What would bring it in scope.** E5, the membership matrix.

## Generated rules

None. No `cata/partition_set.tex` exists, so there is nothing to render.

## Status

**`nothing generated — blocked on E5`**

No decomposition has been attempted, encoded or generated for `partition_set`. This is a scope
...
- `python3 tools/mzn_coverage.py --rank` (run 2026-09-21) → `partition_set` under tier `- out of scope`,
...
- `os.path.isfile("decomps/partition_set.md")` → False. `os.path.isfile("cata/partition_set.tex")` → False.
