...
| **`catalog/all_different_except.md` and its artifact do not share a basename, so `tools/catalog_tex.py` will report a false disagreement.** The entry is `all_different_except.md` (the MiniZinc name); G-1 emitted `cata/alldifferent_except.tex` (the `cata/` convention, matching `alldifferent.tex`). `catalog_tex.py:772` looks up `cata[base]`, `base` comes from the entry filename, and `alldifferent_except` is in neither `catalog_index.py`'s `ALIAS` table nor the release-globals list, so it maps to nothing: expect `all_different_except: entry's Generated row says 1 rules, but there is no cata/all_different_except.tex at all`, plus an unmatched `cata/` file. **The entry is right and the mapping is missing** — one `ALIAS` row (`"alldifferent_except": "all_different_except"`) fixes it. Same class of thing will hit every future artifact whose `cata/` spelling differs from its global's | `tools/catalog_index.py` (`ALIAS`), `tools/catalog_tex.py` | U1 2026-09-22 | open |
| **`validator.ml`'s out-of-scope reason for `among` is half stale after G-1.** It still says `index set D_4 is never defined by the printer (W1-T2)`; there is no `D_4` in `cata/among.tex` any more and `make validate` now stops at the *named* set (`2 rules, UNPARSED: index set: s`). The second clause — "among's count variable appears in no atom" — is still exactly right and is G5. Worth fixing in the same pass as **W1-T18**, which is the larger version of the same problem: the validator cannot see `cata/at_most.tex` or `cata/alldifferent_except.tex` at all | `validator.ml` | U1 2026-09-22 | open |
...
_**U1 RELEASED 2026-09-22.** The four entries G-1's G1/G8 work made stale are re-measured:
`catalog/at_most.md` (`nothing generated — blocked on G1` → **`flagged`**, 1 rule, SOUND and NOT
MINIMAL), `catalog/among.md` (`— blocked on G8` → **`flagged`**, 2 rules, both UNSOUND, with the
unsoundness in a banner above every other section), `catalog/all_different_except.md`
(`— blocked on G8` → **`generated, unvalidated`**, 1 rule, measured sound for `n ≥ 2`) and
`catalog/all_different_except_0.md` (`— blocked on G8` → **`encodable today, not encoded`**;
`DExc (D 2, [EInt 0])` is writable today and nothing writes it). **Every soundness figure in all
four is attributed to G-1's exhaustive check at `n,m ≤ 4`, quoted from the artifacts' `%% CAVEAT`
footers, and explicitly not to `make validate`** — which named none of the two new files (W1-T18).
Nothing outside those four `catalog/*.md` was edited. Two cross-session requests filed above._
