...
| W2-T5 | Decomposition spec corpus: one format-independent file per MiniZinc global, stating the decomposition as maths, the literals it needs and its E-code | TODO | **Added 2026-09-18, and it is the honest parallel axis before the freeze.** Writing a decomposition as maths touches no engine code, so it does not collide the way D-0006 warns about — and **it is the input to W2-T1**: you cannot freeze a format without knowing what it must express. Order families by E-code, E0 first. Per D-0003 these are authored here, not imported | **Family split, wave two (2026-09-18):** W2-A = `CHRISTMAS_LIST.md` §3 value ordering/precedence/symmetry + §4 sequencing and sliding; W2-B = §5 extensional + §6 scheduling; W2-C = §1 alldifferent family + §9 ordering, sorting, channelling. §2 counting is the landed pilot. §7 geometry, §8 graph (E6), §10 sets (E5) stay out of scope. §11 maths/misc and a cross-family shape consolidation are wave three. **Shape-first is the rule, for cost and for quality: specify a decomposition shape once, then list the constraints that instantiate it.** W1-D already found `at_least`/`at_most`/`exactly` are one shape; ~90 in-scope globals will collapse into far fewer shapes than files |
