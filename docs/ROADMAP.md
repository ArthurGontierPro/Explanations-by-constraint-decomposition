...
| W1-T9 | Fix the three defects W1-T8 measured: `allequal` rules 2–3 inverted inequality; `element` loses 4 of 6 rules to unsatisfiable premises; `gcc` rules 1–2 vacuous | TODO | **Added 2026-09-18, all three measured with counterexamples, all three in the decomposition table — one owner, rule-engine file.** `allequal`: counterexample `n=m=2, X=(1,1)`. `element`: `∀t: V=t` and `∀i: I=i` are unsatisfiable. `gcc`: `∀p ∈ [1,n]: O_t ≥ p` means `O_t ≥ n`, contradicting its companion premise |
